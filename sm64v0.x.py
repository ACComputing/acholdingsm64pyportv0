...
def add_fountain(tris, cx, by, cz):
    """Add a decorative fountain in the courtyard."""
    # Base
    add_cylinder(tris, cx, by, cz, 70, 8, 12, STONE_GRAY, STONE_GRAY)
    # Middle tier
    add_cylinder(tris, cx, by + 8, cz, 50, 12, 12, STONE_GRAY, STONE_GRAY)
    # Top bowl
    add_cylinder(tris, cx, by + 20, cz, 35, 6, 12, STONE_GRAY, STONE_GRAY)
    # Center spire
    add_cylinder(tris, cx, by + 26, cz, 8, 20, 8, GOLD, GOLD)
    # Water (blue disc)
    add_cylinder(tris, cx, by + 7, cz, 45, 1, 12, WATER_BLUE, WATER_BLUE)

def add_statue(tris, cx, by, cz, color=STONE_GRAY):
    """Add a simple statue (column with sphere)."""
    add_cylinder(tris, cx, by, cz, 12, 30, 8, color, color)
    add_cylinder(tris, cx, by + 30, cz, 16, 8, 8, color, color)
    # Head
    add_cylinder(tris, cx, by + 38, cz, 10, 10, 8, color, color)
...
class Level:
    ...
    def build(self):
        ...
        # 5. Outer Walls / Corner Towers (Courtyard)
        wall_h = 60
        wall_w = 20
        ...
        # 6. Enhanced Courtyard Details
        courtyard_y = -2  # Slightly above ground
        # Courtyard cobblestone floor (covering area between walls, avoiding walls themselves)
        floor_x0, floor_x1 = -380, 380
        floor_z0, floor_z1 = -130, 480
        floor_w = floor_x1 - floor_x0
        floor_d = floor_z1 - floor_z0
        floor_cx = (floor_x0 + floor_x1) / 2
        floor_cz = (floor_z0 + floor_z1) / 2
        add_box(t, floor_cx, courtyard_y, floor_cz, floor_w, 2, floor_d, COBBLE, COBBLE, COBBLE)

        # Fountain at the center of the courtyard
        add_fountain(t, 0, courtyard_y, 150)

        # Statues flanking the main path to the castle
        add_statue(t, -80, courtyard_y, 50, STONE_GRAY)
        add_statue(t, 80, courtyard_y, 50, STONE_GRAY)
        # Additional statues near the corners
        add_statue(t, -300, courtyard_y, 400, STONE_GRAY)
        add_statue(t, 300, courtyard_y, 400, STONE_GRAY)

        # Decorative hedges or low walls (optional)
        hedge_color = DARK_GREEN
        add_box(t, -200, courtyard_y, 250, 20, 10, 40, hedge_color, hedge_color, hedge_color)
        add_box(t, 200, courtyard_y, 250, 20, 10, 40, hedge_color, hedge_color, hedge_color)

        # 7. Hills (Sloping green banks on sides)
        add_cone(t, -650, -20, 150, 300, 250, 8, GRASS_GREEN)
        add_cone(t, 650, -20, 150, 300, 250, 8, GRASS_GREEN)

        # 8. Trees (around the outer grounds)
        ...
        # 9. Platforms (floating near castle)
